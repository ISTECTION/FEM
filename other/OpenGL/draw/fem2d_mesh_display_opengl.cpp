...
//# include <GLUT/glut.h>
//
//  This is the include statement I need for my usual Unix system.
//
...
//  Purpose:
//
//    MAIN is the main program for FEM2D_MESH_DISPLAY_OPENGL.
//
//  Discussion:
//
//    This program reads information defining a mesh, and displays
//    an image of the mesh using Open GL.
//
//  Usage:
//
//    fem2d_mesh_display_open_gl prefix
//
...
//
{
  ...
  cout << "\n";
  cout << "FEM2D_MESH_DISPLAY_OPENGL:\n";
  cout << "  C++ version\n";
  cout << "\n";
  cout << "  This program reads information defining a mesh\n";
  cout << "  and displays an image of the mesh using OpenGL.\n";
//
...
//
  ...
  {
    cerr << "\n";
    cerr << "FEM2D_MESH_DISPLAY_OPENGL - Fatal error!\n";
    cerr << "  The spatial dimension of the data should be 2.\n";
    exit ( 1 );
  }
  ...
  if ( node_num < 2 )
  {
    cerr << "\n";
    cerr << "FEM2D_MESH_DISPLAY_OPENGL - Fatal error!\n";
    cerr << "  The number of nodes must be at least 2!\n";
    exit ( 1 );
  }
  node_xy = r8mat_data_read ( node_filename, dim_num, node_num );

  xy_min = r82vec_min ( node_num, node_xy );
  xy_max = r82vec_max ( node_num, node_xy );

  node_xy_range[0] = xy_max[0] - xy_min[0];
  node_xy_range[1] = xy_max[1] - xy_min[1];
...
//
  ...
  cout << "\n";
  cout << "  The order ELEMENT_ORDER =            " << element_order << "\n";
  cout << "  The number of elements ELEMENT_NUM = " << element_num << "\n";

  if ( element_order < 3 )
  {
    cerr << "\n";
    cerr << "FEM2D_MESH_DISPLAY_OPENGL - Fatal error!\n";
    cerr << "  The element order must be at least 3!\n";
    exit ( 1 );
  }
  ...
  if ( element_num < 1 )
  {
    cerr << "\n";
    cerr << "FEM2D_MESH_DISPLAY_OPENGL - Fatal error!\n";
    cerr << "  The number of elements must be at least 1!\n";
    exit ( 1 );
  }
...
//
  ...
  glutCreateWindow ( "Polygonal Mesh" );
...
//
//  Draw the element sides, in RED.
//
  glColor3f ( 1.0, 0.0, 0.0 );

  for ( element = 0; element < element_num; element++ )
  {
    order_previous = element_order - 1;
    for ( order = 0; order < element_order; order++ )
    {
      glBegin ( GL_LINES );

        node = element_node[order_previous+element*element_order];
        p[0] = ( float ) node_xy[0+node*2];
        p[1] = ( float ) node_xy[1+node*2];
        glVertex2fv ( p );
